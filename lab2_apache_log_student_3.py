...
endpointSum = endpointCountPairTuple.reduceByKey(lambda count1,count2 : count1+count2)
topTenErrURLs = endpointSum.takeOrdered(10,key=lambda x: -1*x[1])
...
hosts = access_logs.map(lambda log: (log.host, 1)).distinct().reduceByKey(lambda h1,h2: h1 + h2)
uniqueHosts = hosts.filter(lambda (h,c): c == 1)
...
groupedByDay = dayAndHostTuple.map(lambda (x,y) : (x,y),1)#.reduceByKey(lambda x,y: x+y)
...
#avgDailyReqPerHost = sortedByDay.reduce(lambda a, b : a + b) / sortedByDay.count()
#                      <FILL IN>)
...
# TODO: Replace <FILL IN> with appropriate code
#access_logs.filter(lambda log:(log.response_code != 200))
